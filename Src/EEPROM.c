...
//  eeprom_read_block((void*)&global_conf, (void*)0, sizeof(global_conf));
  AT24C16_read_block(0, sizeof(global_conf), (uint8_t*)&global_conf);
...
//  eeprom.readBytes((global_conf.currentSet * sizeof(conf) + sizeof(global_conf), sizeof(conf), (uint8_t*)&conf);
  AT24C16_read_block(global_conf.currentSet * sizeof(conf) + sizeof(global_conf), sizeof(conf), (uint8_t*)&conf);
...
//  eeprom.writeBytes(0, sizeof(global_conf), (uint8_t*)&global_conf);
  AT24C16_write_block(0, sizeof(global_conf), (uint8_t*)&global_conf);
...
void writeParams(uint8_t b) {
    ...
    global_conf.currentSet=0;
  #endif
 // uint8_t sum_min  = calculate_sum((uint8_t*)&conf, sizeof(conf));
 // Serial2.print("calculate_sum= ");  Serial2.println(sum_min, HEX);
...
//  eeprom.writeBytes((global_conf.currentSet * sizeof(conf) + sizeof(global_conf)), sizeof(conf), (uint8_t*)&conf);
  AT24C16_write_block((global_conf.currentSet * sizeof(conf) + sizeof(global_conf)), sizeof(conf), (uint8_t*)&conf);
  readEEPROM();
//  if (b == 1) blinkLED(15,20,1);
  #if defined(BUZZER)
...
void LoadDefaults() {
  uint8_t i;
    ...
    conf.pid[PIDPITCH].P8    = 40; 	 conf.pid[PIDPITCH].I8    = 0; conf.pid[PIDPITCH].D8    = 100;
